...
<<<<<<< HEAD:src/pages/Changelogs.vue
						Create Changelog
=======
						Create Template
>>>>>>> c673ba5 (Update libraries and other functionalities):src/pages/templates/Index.vue
					</t-button>
				</span>
			</div>
		</div>
						...
						<div class="border-b border-gray-200">
							<div class="mt-2 -mb-px flex space-x-8" aria-label="Tabs">
								<div
									...
									:key="tab.name"
									:href="tab.href"
									:class="[tab.id === selectedType ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200', 'whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm cursor-pointer']"
									@click="selectedType = tab.id">
									{{ tab.name }}
									<span v-if="tab.count" :class="[tab.id === selectedType ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-900', 'hidden ml-2 py-0.5 px-2.5 rounded-full text-xs font-medium md:inline-block']">{{ tab.count }}</span>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
						...
						<table class="min-w-full divide-y divide-gray-200">
							...
							<tr v-for="item in items" :key="item.id">
								...
								<td class="px-6 py-4 whitespace-nowrap hidden sm:table-cell">
									<p class="text-xs text-gray-500" v-if="item.start_date">
										<time :datetime="item.start_date">{{ __dateFormat(item.start_date) }}</time>
									</p>
								</td>
								<td class="px-6 py-4 whitespace-nowrap hidden sm:table-cell">
									<p class="text-xs text-gray-500" v-if="item.end_date">
										<time :datetime="item.end_date">{{ __dateFormat(item.end_date) }}</time>
									</p>
								</td>
								<td class="px-6 py-4 whitespace-nowrap hidden sm:table-cell">
		...
		<div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
			...
			<div class="mt-2">
				<p class="text-sm text-gray-500">
					Are you sure you want to delete this changelog? This data will be permanently removed from our servers forever.
...
// Statics
import { TYPE_CHANGELOGS } from '@/databags/changelog';
...
import SideForm from '@/components/changelogs/SideForm.vue';
import SideDetails from '@/components/changelogs/SideDetails.vue';
...
export default {
	...
	methods: {
		fetchList(isReset = false) {
			...
			changelogApi.getList(params, callback, errorCallback);
		},
		setPage(page) {
		...
		approveRemove() {
			...
			changelogApi.delete(selectedId, callback, errorCallback);
		},
		onRemove(selectedId) {
